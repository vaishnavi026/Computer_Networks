...
int main(void)
{
    ...
    if (sendto(sockfd, buff_command, strlen(buff_command) , 0 , (struct sockaddr *) &si_other, sizeof(si_other))==-1) // write(sockfd, buff_command, 2);
    {
        die("sendto()");
    }
  	...
  	if(command == 1 || command == 2)   // We need to specify the offset
    {
        ...
        if (sendto(sockfd, buff_offset, strlen(buff_offset) , 0 , (struct sockaddr *) &si_other, sizeof(si_other))==-1)  //write(sockfd, buff_offset, 10);    	{
        	die("sendto()");
    	}
    }
